...
from get_amazon import get_amazon_reviews_bs4 # Moved to the top!
import json
...
def is_valid_amazon_link(link):
    if not link:
        return False
    link = link.strip()
    # Added "/product-reviews/" to the accepted formats
    return "amazon." in link and ("/dp/" in link or "/product/" in link or "/product-reviews/" in link)
...
def sany():
    ...
    if option_type == 'yt':
        ...
        p_comm = len(df[df['sentiment'] == 0])
        n_comm = len(df[df['sentiment'] == 1])
    ...
    # ==========================================
        ...
        get_amazon_reviews_bs4(link)
        print("Running ML Preprocessing on Amazon data...")
        success = process_link() 
        if not success:
            flash('Failed to extract reviews. Amazon likely blocked the request, or there are no text reviews.')
            return render_template('index.html') # Reuses the exact same ML model!
